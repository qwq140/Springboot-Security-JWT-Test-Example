...
public class SecurityConfig {
    ...
    @Bean // IoC 컨테이너에 BCryptPasswordEncoder() 객체가 등록됨. @Configuration 내부에서만 @Bean 작동함
    ...
    public SecurityFilterChain filterChain(HttpSecurity http) throws Exception {
        ...
        http.csrf().disable(); // enable이면 postman 작동안함
        ...
        http.authorizeRequests()
                ...
                .antMatchers("/api/admin/**").hasRole(""+ UserRoleType.ADMIN) // 최근 공식문서에서는 ROLE_ 안붙여도 됨
    ...
    public CorsConfigurationSource configurationSource(){
        ...
        configuration.addExposedHeader("Authorization"); // 옛날에는 이게 디폴트
        UrlBasedCorsConfigurationSource source = new UrlBasedCorsConfigurationSource();
        source.registerCorsConfiguration("/**",configuration); // 모든 주소 요청에 설정해주겠다.
        return source;
    }
}
